...
import de.freitag.stefan.sht21.model.Command;
...
public final class SHT21Impl extends AbstractSHT21 {
  ...
  /**
   * Calculates the relative humidity. The relative humidity RH is obtained by the following formula
   * (result in %RH), no matter which resolution is chosen: RH = - 6 + 125 x (S_RH/2^16)
   *
   ...
   * @return {@code true}, if calculated and reported checksum match. Otherwise, {@code false} is
   *     returned.
   */
  ...
  private void softReset() {
    try {
      log.debug(
          "Writing byte "
              + String.format("0x%02X", Command.SOFT_RESET.getCommandByte())
              + " to device "
              + this.device);
  ...
  public Resolution getResolution() {
    ...
    try {
      log.debug(
          "Writing byte "
              + String.format("0x%02X", Command.USER_REG_R.getCommandByte())
              + " to device "
              + this.device);
      ...
      case HUMIDITY:
        {
          return Measurement.builder()
              ...
              .value(
                  BigDecimal.valueOf(this.measurePollHumidity())
      ...
      case TEMPERATURE:
        {
          return Measurement.builder()
              .measuredAt(Instant.now().toEpochMilli())
              .value(
                  BigDecimal.valueOf(this.measurePollTemperature())
                      .setScale(2, RoundingMode.HALF_EVEN))
              .unit("Celsius")
              .build();
        }
