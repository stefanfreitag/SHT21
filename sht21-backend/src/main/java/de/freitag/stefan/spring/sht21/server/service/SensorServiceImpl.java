...
public class SensorServiceImpl implements SensorService {
  ...
  public SensorServiceImpl(
      final @NonNull SensorRepository repository,
  ...
  public List<MeasurementDTO> getMeasurements(final String uuid, final Long from, final Long to) {
    List<Measurement> m =
        this.measurementRepository.findByMeasuredAtBetweenAndSensor_Id(from, to, uuid);
  ...
  public MeasurementDTO addMeasurement(
      @NonNull final String uuid, @NonNull final MeasurementDTO measurementDTO) {
  ...
  private MeasurementDTO convertToDto(
      de.freitag.stefan.spring.sht21.server.domain.model.Measurement post) {
  ...
  private de.freitag.stefan.spring.sht21.server.domain.model.Measurement convertToEntity(
      MeasurementDTO postDto) {
    de.freitag.stefan.spring.sht21.server.domain.model.Measurement post =
        modelMapper.map(
            postDto, de.freitag.stefan.spring.sht21.server.domain.model.Measurement.class);
    return post;
  }
}
